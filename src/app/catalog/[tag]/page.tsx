...
async function fetchProducts(tag: string): Promise<Product[]> {
  const res = await fetch(`http://localhost:4000/api/products/tag/${tag}`);
  if (!res.ok) {
...
const CatalogPage: React.FC<CatalogPageProps> = ({ params }) => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        const fetchedProducts = await fetchProducts(params.tag);
